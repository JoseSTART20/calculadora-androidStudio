...
class MainActivity : AppCompatActivity() {
    ...
    fun calcular(view : View){
        ...
        var concatenar=TxtResul?.text.toString()+textoBoton
        var concatenarSinCeros=quitarCerosIzquierda(concatenar)
        if (textoBoton=="="){
            var resultado=0.0
            try {
                resultado=eval(TxtResul?.text.toString())
                TxtResul?.text=resultado.toString()
            }catch (e:Exception){
                TxtResul?.text=e.toString()
            }
        }else if (textoBoton=="RESET"){
            TxtResul?.text="0"
        }else{
            TxtResul?.text=concatenarSinCeros
        }
    }
